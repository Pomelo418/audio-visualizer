...
- **Mood images may not load in Safari** — this is a Safari-side content blocker (extensions like AdGuard, 1Blocker, Wipr, etc.) blocking `loremflickr.com`, not a bug in the app. It works fine in Firefox and Chrome without such extensions. To fix it in Safari: check Safari → Settings → Extensions for a content/ad blocker and either disable it or allow it for this site (via the "aA" icon in the address bar → Settings for This Website). If that doesn't help, open Develop → Show Web Inspector → Network tab, filter for "loremflickr," and check whether requests are being blocked outright.
